...
`make_moons`, `spiral`, Iris 로딩이 기대한 shape, label, split 동작을 내는지 확인합니다.
...
왜 지금 더 중요해졌나:
프로젝트가 초기 `make_moons` / Iris 수준을 넘어 다층 MLP와 더 큰 synthetic dataset 실험으로 넘어가고 있기 때문입니다. `spiral`은 대형 벤치마크는 아니지만, 2D 시각화 가능성을 유지한 채 search-space 증가와 multi-class 거동을 보여주기에는 충분합니다.

### 2. 다층 모델 / Gradient 체크

이 테스트는 더 깊은 MLP 구성이 다음을 만족하는지 봅니다.

- parameter layout이 정상적으로 만들어지는지
- forward pass가 돌아가는지
- gradient vector shape가 맞는지

이 테스트가 막는 것:
- 단일 hidden layer 전제가 다층 코드에 남아 있는 문제
- parameter pack/unpack 깨짐
- SGD baseline 경로 회귀
### 3. Trainer 스모크 테스트
...
이제 여기에 `T-WFC vs SGD` metrics board, 2D boundary board, comparison GIF 출력 경로도 포함됩니다.
...
### 7. SGD baseline 스모크 테스트

이 테스트는 전통적인 `numpy` SGD 경로가 더 깊은 MLP에서 적어도 하나의 안정적인 데이터셋에서는 실제로 개선되는지 확인합니다.

이 테스트가 막는 것:
- baseline 자체의 회귀
- 일반화된 MLP의 backprop wiring 문제
- T-WFC가 무작위 초기화와만 비교되고 실제 optimizer와는 비교되지 않는 상태

하지만 아직 보장하지 않는 것:
- T-WFC가 SGD보다 낫다는 결론
- 큰 규모에서 비교가 이미 충분히 공정하다는 결론
- deeper model에서 hard-weight 품질이 shadow-weight 품질을 잘 따라간다는 보장
...
이 명령은 다음을 한 번에 태우기 때문에 “확장 경로가 아직 말이 되나?”를 보는 데 좋습니다.
- 더 깊은 MLP 경로
- T-WFC용 automatic initial jitter
- SGD baseline 경로
- side-by-side CLI 요약
